...
```sh
zig build score-workbench -- tutorial src/assets/scores/flowing-six-four-piano-lab.mxl
zig build score-workbench -- compile-glyph-atlas ATLAS.csv ATLAS.rgba src/render/glyph_atlas.zig src/render/assets/glyph-atlas.rgba
zig build score-workbench -- inspect SCORE.mxl [--measure N]
...
zig build score-workbench -- audit-measures SCORE.mxl EVIDENCE.json --anchors ANCHORS.json --output REPORT.json
...
zig build score-workbench -- compare-performance REFERENCE.wav LOCAL-RANGE.wav --score SCORE.mxl --anchors ANCHORS.json --start-measure N --end-measure N --candidate-start-beat N --output REPORT.json
...
zig build && ./zig-out/bin/score-sampler-workbench verify REPORT.json EVIDENCE.wav PIANO.sfz
zig build && ./zig-out/bin/score-sampler-workbench inspect-pack PIANO.sfz REPORT.json
zig build && ./zig-out/bin/score-sampler-workbench portable-pack PIANO.sfz OUTPUT.scorebank
zig build && ./zig-out/bin/score-sampler-workbench portable-verify OUTPUT.scorebank EVIDENCE.wav
```
...
The verifier emits schema-3 latency, spectral, velocity, pedal, acoustic-layer,
replay, and stress evidence. `inspect-pack` uses the shared Zig SFZ normalizer,
opens every referenced WAV/FLAC asset, validates its lossless container
metadata, hashes its content, reports missing/invalid paths, and writes a
deterministic pack identity. Both remain subcommands of the same consolidated
Zig executable.
